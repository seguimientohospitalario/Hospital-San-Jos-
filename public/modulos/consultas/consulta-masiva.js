document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnGenerar = document.getElementById('btn-generar-aleatorios');
    ...
    const saveState = () => {
        try { 
            sessionStorage.setItem('cm_accumulated', JSON.stringify(accumulatedResults));
            // Guardar IDs seleccionados hoy para que Validación Global los ignore
    ...
    const restoreState = async () => {
        try {
            const saved = sessionStorage.getItem('cm_accumulated');
            if (saved) { 
        ...
        accumulatedResults.forEach(p => {
            ...
            tr.innerHTML = `
                ...
                <td style="text-align:center;">
                    <button class="btn-module primary" style="display:${btnDisplay}; border:none; border-radius:6px; padding:6px 12px; cursor:pointer; font-size:13px;" onclick="window.location.href='../seguimiento/verificacion-paciente.html?dni=${p.dni}&from=masiva'">
    ...
    btnGenerar.addEventListener('click', async () => {
        ...
        // Usamos un universo de 300 para barajar y dar sensación de aleatoriedad
        ...
        if (data.length === 0) {
            showToast('No hay pacientes pendientes de validación hoy.', true);
            return;
        }
    ...
    btnClear.addEventListener('click', () => {
        accumulatedResults = [];
        selectedPatients = [];
        sessionStorage.removeItem('cm_accumulated');
        renderTable();
    });
    ...
    btnValidar.addEventListener('click', async () => {
        ...
        saveState();
        renderTable();
        showToast('Validación masiva completada');
    });
